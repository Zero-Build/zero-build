...
import { getPrivacyPolicy } from "@/sanity/sanity-utils";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const privacyRes = await getPrivacyPolicy();
        console.log("Privacy policy response:", privacyRes);
        // Handle if privacyRes is an array
        const policy = Array.isArray(privacyRes) ? privacyRes[0] : privacyRes;
        setPrivacyPolicy(policy || null);
      } catch (err) {
    ...
    return (
      <div className="mt-[100px]">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center">Loading privacy policy...</div>
        </div>
      </div>
    );
  }
          ...
          {privacypolicy ? (
            <div>
              ...
              <div className="text-gray-600 prose prose-sm max-w-none">
                {Array.isArray(privacypolicy.description) && privacypolicy.description.length > 0 ? (
                  <PortableText value={privacypolicy.description} />
                ) : (
